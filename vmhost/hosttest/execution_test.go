...
import (
	...
	"github.com/kalyan3104/k-chain-vm-go/config"
	"github.com/kalyan3104/k-chain-vm-go/executor"
	executorwrapper "github.com/kalyan3104/k-chain-vm-go/executor/wrapper"
	vmMath "github.com/kalyan3104/k-chain-vm-go/math"
			...
			verify.Ok().
				...
				Transfers(
					test.CreateTransferEntry(test.ParentAddress, test.ParentTransferReceiver, 1).
						WithData(test.ParentTransferData).
						WithValue(big.NewInt(test.ParentTransferValue)),
					test.CreateTransferEntry(test.ParentAddress, test.ChildTransferReceiver, 2).
	...
	test.BuildInstanceCallTest(t).
		...
		AndAssertResults(func(host vmhost.VMHost, stubBlockchainHook *contextmock.BlockchainHookStub, verify *test.VMOutputVerifier) {
			verify.Ok().
				...
				Transfers(
					test.CreateTransferEntry(test.ChildAddress, test.ChildTransferReceiver, 2).
		...
		AndAssertResults(func(host vmhost.VMHost, stubBlockchainHook *contextmock.BlockchainHookStub, verify *test.VMOutputVerifier) {
			verify.Ok().
				...
				Logs(vmcommon.LogEntry{
					...
					Data:       vmcommon.FormatLogDataForCall("DeploySmartContract", "init", [][]byte{{0}}),
				})
		})
}
